...
def load_matern_1d(base_dir='./dataset', position_encoding=False, m=3):
...
def load_agw_1d(base_dir='./dataset', get_feats=False, position_encoding=False, m=3):
    ...
    if get_feats:
        if position_encoding:
            F = position_encode(F, m)
        return F[:, None, :], Y[:, None, :]
...
# CAN NOT LOAD, lack some file
def load_andrew_1d(base_dir='./dataset', position_encoding=False, m=3):
...
def load_dun_1d(base_dir='./dataset', position_encoding=False, m=3):
    if not path.exists(base_dir + '/dun_1d_data/'):
        mkdir(base_dir + '/dun_1d_data/')
        ...
        np.save(base_dir + '/dun_1d_data/dun_1d_data.npy', xy)

        if position_encoding:
            X_train = position_encode(X_train, m)
            X_test = position_encode(X_test, m)

        return X_train[:, None, :], y_homo_train[:, None, :], X_test[:, None, :], y_homo_test[:, None, :]

    xy = np.load(base_dir + '/dun_1d_data/dun_1d_data.npy')
