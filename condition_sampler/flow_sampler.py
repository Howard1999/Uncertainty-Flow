...
class FlowSampler:
    ...
    def fit(self, x, epoch=10, lr=1e-2, save_model=False, save_dir=None, batch=32) -> list:
        ...
        for i in tqdm(range(epoch)):
            ...
            if save_model and save_dir is not None:
                path = os.path.join(save_dir, 'epoch' + str(i) + '.pt')
                self.save(path)
        
        if save_model and save_dir is not None:
            os.symlink(path, os.path.join(save_dir, 'latest.pt'))
    ...
    def load(self, path) -> None:
        self.prior.load_state_dict(torch.load(path, map_location='cuda:'+str(self.gpu)))
